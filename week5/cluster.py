...
import time
from descriptors import DCT, twod_hist, celled_2dhist, mrhm, LBP
...
def compute_std_centers(museum_desc, kmeans):
    stds = [np.zeros((2048)) for _ in range(kmeans.labels_.max()+1)]
    for i, desc in enumerate(museum_desc):
        saved_imgs = 0
        c_class = kmeans.labels_[i]
        stds[c_class] += ((kmeans.cluster_centers_[c_class]-desc)*(kmeans.cluster_centers_[c_class]-desc))/(kmeans.labels_ == c_class).astype(np.uint8).sum()
    return stds
...
def dct_clustering(images, n_clusters=10, top_representative=5):
    ...
    for museum_image in tqdm(images):
        ...
        museum_textures.append(DCT(museum_image, num_blocks=4, number_coefficients=128))

    feat_np = np.asarray(museum_textures, dtype=np.float64)
    #feat_np = ((feat_np-feat_np.mean(0))/(feat_np.std(0)))
    feat_np = whiten(feat_np)
    print("\n Performing KMeans")
    kmeans = AgglomerativeClustering(n_clusters=n_clusters, linkage="complete").fit(feat_np)
    ...
    stds = compute_std_centers(feat_np, kmeans)
...
def dct_clustering_color(images, n_clusters=10, top_representative=5):
    ...
    for museum_image in tqdm(images):
        museum_image = cv2.imread(museum_image)
        museum_textures.append(np.concatenate([DCT(museum_image, num_blocks=12, number_coefficients=128), mrhm(museum_image, None, 4)]))
...
def lbp_clustering(images, n_clusters=10, top_representative=5):
    ...
    for museum_image in tqdm(images):
        museum_image = cv2.imread(museum_image)
        museum_textures.append(np.concatenate([LBP(museum_image, num_blocks=12)]))
    ...
    for i, k_l in enumerate(kmeans.labels_):
        dst_dir = '/home/sergio/MCV/M1/Practicas/DB/Clustering/LBP' + str(k_l)
        if not os.path.exists(dst_dir):
            os.makedirs(dst_dir)
        shutil.copy(filenames[i], dst_dir)
    ...
    save_rep_imgs(top_k_imgs, kmeans, "LBP")
...
def color_clustering(images, n_clusters=10, top_representative=5):
    ...
    for museum_image in tqdm(images):
        museum_image = cv2.imread(museum_image)
        museum_textures.append(mrhm(museum_image, num_blocks=4))
